...
class Metabolism(six.with_metaclass(ChemblModelMetaClass, ChemblCoreAbstractModel)):
    ...
    met_id = ChemblPositiveIntegerField(primary_key=True, length=9, help_text=u'Primary key')
    ...
    pathway_id = ChemblPositiveIntegerField(length=9, blank=True, null=True, choices=PATHWAY_ID_CHOICES, help_text=u'Identifier for the metabolic scheme/pathway (may be multiple pathways from one source document)')
    ...
    tax_id = ChemblPositiveIntegerField(length=11, blank=True, null=True, choices=TAX_ID_CHOICES, help_text=u'NCBI Tax ID for the organism in which this metabolic reaction occurs')
    ...
    class Meta(ChemblCoreAbstractModel.Meta):
        unique_together = ( ("drug_record", "substrate_record", "metabolite_record", "pathway_id", "target"),  )
...
class MetabolismRefs(six.with_metaclass(ChemblModelMetaClass, ChemblCoreAbstractModel)):
    ...
    metref_id = ChemblPositiveIntegerField(primary_key=True, length=9, help_text=u'Primary key')
